...
import com.google.crypto.tink.{InsecureSecretKeyAccess, Key, Parameters, SecretKeyAccess}
...
object ChaCha20Poly1305ProtoSerializationTest {
  private val TYPE_URL = "type.googleapis.com/google.crypto.tink.ChaCha20Poly1305Key"
  ...
  private def createInvalidKeySerializations = try Array[ProtoKeySerialization](
    ...
    //    1479),
    // Unknown prefix
    ProtoKeySerialization.create(TYPE_URL, com.google.crypto.tink.proto.ChaCha20Poly1305Key.newBuilder.setKeyValue(KEY_BYTES_32_AS_BYTE_STRING).build(), KeyMaterialType.SYMMETRIC, OutputPrefixType.UNKNOWN_PREFIX, 1479), // Bad Key Length
    ProtoKeySerialization.create(TYPE_URL, com.google.crypto.tink.proto.ChaCha20Poly1305Key.newBuilder.setKeyValue(ByteString.copyFrom(new Array[Byte](16))).build(), KeyMaterialType.SYMMETRIC, OutputPrefixType.TINK, 1479))
  catch {
    case e: GeneralSecurityException =>
      throw new RuntimeException(e)
  }

  @DataPoints(Array("invalidKeySerializations"))
  def INVALID_KEY_SERIALIZATIONS: Array[ProtoKeySerialization] = createInvalidKeySerializations
}

@RunWith(classOf[Theories])
@SuppressWarnings(Array("UnnecessarilyFullyQualified")) final class ChaCha20Poly1305ProtoSerializationTest {
  ...
  def serializeParseKey_tink(): Unit = {
    val key = ChaCha20Poly1305Key.create(ChaCha20Poly1305Parameters.Variant.TINK, ChaCha20Poly1305ProtoSerializationTest.KEY_BYTES_32, 123)
    val protoChaCha20Poly1305Key = com.google.crypto.tink.proto.ChaCha20Poly1305Key.newBuilder.setKeyValue(ChaCha20Poly1305ProtoSerializationTest.KEY_BYTES_32_AS_BYTE_STRING).build()
    val serialization = ProtoKeySerialization.create("type.googleapis.com/google.crypto.tink.ChaCha20Poly1305Key", protoChaCha20Poly1305Key, KeyMaterialType.SYMMETRIC, OutputPrefixType.TINK, /* idRequirement= */ 123)
  ...
  def serializeParseKey_crunchy(): Unit = {
    val key = ChaCha20Poly1305Key.create(ChaCha20Poly1305Parameters.Variant.CRUNCHY, ChaCha20Poly1305ProtoSerializationTest.KEY_BYTES_32, 123)
    val protoChaCha20Poly1305Key = com.google.crypto.tink.proto.ChaCha20Poly1305Key.newBuilder.setKeyValue(ChaCha20Poly1305ProtoSerializationTest.KEY_BYTES_32_AS_BYTE_STRING).build()
    val serialization = ProtoKeySerialization.create("type.googleapis.com/google.crypto.tink.ChaCha20Poly1305Key", protoChaCha20Poly1305Key, KeyMaterialType.SYMMETRIC, OutputPrefixType.CRUNCHY, /* idRequirement= */ 123)
    val serialized = SerializationRegistry.serializeKey(key, classOf[ProtoKeySerialization], InsecureSecretKeyAccess.get)
    val parsed = SerializationRegistry.parseKey(serialization, InsecureSecretKeyAccess.get)
    assertTrue(parsed.equalsKey(key))
  }
  ...
  def parseKey_legacy(): Unit = {
    val serialization = ProtoKeySerialization.create(ChaCha20Poly1305ProtoSerializationTest.TYPE_URL, com.google.crypto.tink.proto.ChaCha20Poly1305Key.newBuilder.setKeyValue(ChaCha20Poly1305ProtoSerializationTest.KEY_BYTES_32_AS_BYTE_STRING).build(), KeyMaterialType.SYMMETRIC, OutputPrefixType.LEGACY, 1479)
    // Legacy keys are parsed to crunchy
    val parsed = SerializationRegistry.parseKey(serialization, InsecureSecretKeyAccess.get)
    assertThat(parsed.getParameters.asInstanceOf[ChaCha20Poly1305Parameters].getVariant).isEqualTo(ChaCha20Poly1305Parameters.Variant.CRUNCHY)
  }

  @Test
  @throws[Exception]
  def testSerializeKeys_noAccess_throws(): Unit = {
    val key = ChaCha20Poly1305Key.create(ChaCha20Poly1305ProtoSerializationTest.KEY_BYTES_32)
